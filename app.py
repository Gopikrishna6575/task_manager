...
def empregistration():
     if session.get('user'):
        if request.method == 'POST':
            ...
            cursor = mysql.connection.cursor()
            cursor.execute('SELECT count(*) FROM employee WHERE empemail=%s',[empemail])
            count= cursor.fetchone()[0]
            #print(count)
            if count!=0:
                return "mail already exists!"
            else:
                cursor.execute('INSERT INTO employee VALUES ( %s,%s,%s,%s,%s)', (ename,empdept,empemail,emppassword,session.get('user')))
                mysql.connection.commit()
                cursor.close()
...
def emplogin():
    ...
    if request.method=='POST':
        ...
        emppassword=request.form['emppassword']
        cursor=mysql.connection.cursor()
        cursor.execute('select count(*) from employee where empemail=%s and emppassword=%s',[empemail,emppassword])
        count=cursor.fetchone()[0]
        if count==0:
...
def empforgot():
    if request.method=='POST':
        ...
        if (empemail,) in edata:
            cursor.execute('select %s from employee',[empemail])
            deta=cursor.fetchone()[0]
            print(deta)
            cursor.close()
            subject=f'Reset Password for {deta}'
...
def empcreatepassword(token):
    try:
        ...
        if request.method=='POST':
            ...
            if npass==cpass:
                cursor=mysql.connection.cursor()
                cursor.execute('update employee set emppassword=%s where empemail=%s',[npass,empemail])
                mysql.connection.commit()
                flash('Password reset Successfully')
...
def addtask():
    if session.get('user'):
        ...
        cursor = mysql.connection.cursor()
        cursor.execute('select * from employee where addedby=%s',[session.get('user')])
